...
class ModelTree(object):
    ...
    def __init__(self, model, header, max_depth=5, min_samples_leaf=10,
                 search_type="greedy", n_search_grid=100, j_feature_range=None,
                 know_where_split=True):
    ...
    def fit(self, X, y, verbose=False):
        ...
        def _build_tree(X, y, j_feature_range):
            ...
            def _create_node(X, y, depth, container):
                ...
                header = self.header
                variables = [header[i] for i in range(len(header))]
                model_print = node["model"].to_string(variables)
