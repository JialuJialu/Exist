...
class ModelTree(object):
    ...
    def __init__(self, model, max_depth=5, min_samples_leaf=10,
                 search_type="greedy", n_search_grid=100, j_feature_range=None, know_where_split=True):
        ...
        if j_feature_range is None:
            assert not know_where_split
        self.j_feature_range = j_feature_range
        self.know_where_split = know_where_split
        self.tree = None
    ...
    # Fit
    # ======================
    def fit(self, X, y, trial=0, verbose=False):
        ...
        def _build_tree(X, y, j_feature_range):
            ...
            def _split_traverse_node(node, container):
                ...
                result = _splitter(node, model,
                                   ...
                                   know_where_split=know_where_split,
                                   max_depth=max_depth,
                                   min_samples_leaf=min_samples_leaf,
                                   search_type=search_type,
                                   n_search_grid=n_search_grid, t=trial)
        ...
        # Construct tree
        self.tree = _build_tree(X, y, j_feature_range)
    ...
    def predict(self, X):
        ...
        _, Nfeatures = X.shape
        if self.know_where_split:
            X_used = X
            # X_used = X[:, np.delete(
            #     np.arange(0, Nfeatures), self.j_feature_range)]
        else:
            X_used = X
        y_pred = np.array([_predict(self.tree, x) for x in X_used])
        return y_pred
    ...
    def explain(self, X, header):
        ...
        def _explain(node, x, explanation):
            ...
            if no_children:
                if self.know_where_split:
                    # used_features = [header[i] for i in range(
                    #     len(header)) if i not in j_feature_range]
                    used_features = header
                else:
                    used_features = header
                explanation.append(node["model"].to_string(used_features))
                return explanation
            else:
                if x[node["j_feature"]] <= node["threshold"]:  # x[j] < threshold
                    explanation.append("{} = {:.6f} <= {:.6f}".format(
        ...
        def build_graphviz_recurse(node, parent_node_index=0, parent_depth=0, edge_label=""):
            ...
            if node["children"]["left"] is None and node["children"]["right"] is None:
                ...
                if self.know_where_split:
                    ...
                    #     len(feature_names)) if i not in j_feature_range]
                    used_features = feature_names
                else:
                    used_features = feature_names
                explain_str = "\\n {}".format(
                    node["model"].to_string(used_features))
            else:
                threshold_str = "{} <= {:.1f}\\n".format(
            ...
            label_str = "{} n_samples = {}\\n loss = {:.6f} {}".format(
                threshold_str, node["n_samples"], node["loss"], explain_str)
    ...
    # def plot_fitting_hist(self, output_filename):
    #     hist = self.fitting_history
    #     nodes = list(hist.keys())
    #     n = len(nodes)
    #     fig, axes = plt.subplots(n, 1, figsize=(100, 30*n))
    #     for i in range(n):
    #         node = nodes[i]
    #         x = np.arange(len(hist[node]["y"]))
    #         axes[i].set_xticks(x)
    #         xticklabels = np.array([[round(q, 2) for q in row]
    #                                 for row in hist[node]["X"]])
    #         axes[i].set_xticklabels(xticklabels, fontsize=7, rotation=90)
    #         axes[i].plot(x, hist[node]["y"], 'x', label="y")
    #         axes[i].plot(x, hist[node]["y_pred"], '-', label="y_pred")
    #         axes[i].set(xlabel='data points', ylabel='y',
    #                     title='fitting node {}'.format(node))
    #         axes[i].legend()
    #     if self.testing_ground_inv:
    #         fig.suptitle("Using ground inv to predict y")
    #     else:
    #         fig.suptitle("Using learned inv to predict y")
    #     fig.savefig("{}.png".format(output_filename))
# ***********************************
#
# Side functions
#
...
def _splitter(node, model, j_feature_range=None,
              know_where_split=False,
              max_depth=5, min_samples_leaf=10,
              search_type="greedy", n_search_grid=100, t=0):
    ...
    j_feature_best = None
    threshold_best = None
    if j_feature_range is None or (not know_where_split):
        split_range = [i for i in range(d)]
    else:
        split_range = j_feature_range

    # Perform threshold split search only if node has not hit max depth
    if (depth >= 0) and (depth < max_depth):
        import pandas as pd
        df = pd.DataFrame(columns=["did_split",
                                   "loss",
                                   "models",
                                   "data",
                                   "j_feature",
        ...
        for j_feature in split_range:
            ...
            for threshold in threshold_search:
                ...
                dicti = {}
                dicti = {"did_split": True, "loss": loss_split, "models": [model_left, model_right], "data": [
                    (X_left, y_left), (X_right, y_right)], "j_feature": j_feature, "threshold": threshold}
    ...
    if not (df.empty):
        result = {"did_split": df["did_split"][t], "loss": df["loss"][t], "models": df["models"][t],
                  "data": df["data"][t], "j_feature": df["j_feature"][t], "threshold": df["threshold"][t], "N": N}
